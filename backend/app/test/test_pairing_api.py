...
class TestGeneratePairingCode:
    ...
    def test_generate_code_returns_existing_active(self, authenticated_client, create_test_user, db_session):
        """Test that generating a code when one exists returns the existing code."""
        client, caregiver = authenticated_client
        user = create_test_user(caregiver.id)
        
        # Generate first code
        response1 = client.post("/api/pairing/generate", json={
            "user_id": str(user.id)
        })
        ...
        response2 = client.post("/api/pairing/generate", json={
            "user_id": str(user.id)
        })
    ...
    def test_generate_code_unauthorized_user(self, authenticated_client, create_test_user, create_test_caregiver):
    ...
    def test_generate_code_without_auth(self, client, create_test_user, create_test_caregiver):
    ...
    def test_generate_code_expiration_time(self, authenticated_client, create_test_user):
        ...
        # Code should expire in approximately 24 hours
...
class TestVerifyPairingCode:
    ...
    def test_verify_code_success(self, authenticated_client, create_test_user, db_session):
        """Test successful pairing code verification."""
        client, caregiver = authenticated_client
        user = create_test_user(caregiver.id)
        
        # Generate code
        generate_response = client.post("/api/pairing/generate", json={
            "user_id": str(user.id)
        })
    ...
    def test_verify_code_expired(self, authenticated_client, create_test_user, db_session):
        ...
        expired_code.expires_at = datetime.now(timezone.utc) - timedelta(hours=1)  # Expired 1 hour ago
    ...
    def test_verify_code_already_used(self, authenticated_client, create_test_user, db_session):
    ...
    def test_verify_code_case_insensitive(self, authenticated_client, create_test_user):
        ...
        response_lower = client.post("/api/pairing/verify", json={
            "code": code.lower()
        })
        
        # Try different case combination
        response_mixed = client.post("/api/pairing/verify", json={
            "code": code.upper() if code[0].islower() else code.lower()
        })
    ...
    def test_verify_code_no_auth_required(self, client, create_test_user, create_test_caregiver, db_session):

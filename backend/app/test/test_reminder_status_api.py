...
class TestGetCurrentStatus:
    ...
    def test_get_current_status_success(
        ...
        status = create_test_reminder_status(
            reminder_id=reminder.id, status=ReminderStatusEnum.PENDING.value
        )
    ...
    def test_get_current_status_latest(
        ...
        create_test_reminder_status(
            reminder_id=reminder.id, status=ReminderStatusEnum.POSTPONED.value
        )
        latest_status = create_test_reminder_status(
            reminder_id=reminder.id, status=ReminderStatusEnum.DONE.value
        )
    ...
    def test_get_current_status_not_found(
        self, authenticated_client, create_test_user, create_test_reminder
    ):
...
class TestGetStatusHistory:
    ...
    def test_get_status_history_unauthenticated(
        self, client, create_test_user, create_test_reminder, create_test_caregiver
    ):
...
class TestUpdateReminderStatus:
    ...
    def test_update_status_to_done(
        ...
        response = client.put(f"/api/reminder-status/{reminder.id}", json=update_data)
    ...
    def test_update_status_missing_field(
        ...
        response = client.put(f"/api/reminder-status/{reminder.id}", json={})
    ...
    def test_update_status_nonexistent_reminder(self, authenticated_client):
        ...
        update_data = {"status": "DONE"}
        response = client.put(
            f"/api/reminder-status/{fake_reminder_id}", json=update_data
        )
    ...
    def test_update_status_invalid_uuid(self, authenticated_client):
        ...
        update_data = {"status": "DONE"}
        response = client.put("/api/reminder-status/invalid-uuid", json=update_data)
...
class TestGetValidStatuses:
    ...
    def test_get_valid_statuses_success(self, client):
        ...
        assert ReminderStatusEnum.MISSED.value in data
    ...
    def test_get_valid_statuses_order(self, client):
        ...
        expected_statuses = [
            ...
            ReminderStatusEnum.MISSED.value,
        ]
...
class TestReminderStatusIntegration:
    ...
    def test_complete_status_lifecycle(
        ...
        response = client.put(
            f"/api/reminder-status/{reminder.id}", json={"status": "POSTPONED"}
        )
        assert response.status_code == 200
        ...
        # 5. Update to DONE
        response = client.put(
            f"/api/reminder-status/{reminder.id}", json={"status": "DONE"}
        )
        assert response.status_code == 200
    ...
    def test_multiple_reminders_different_statuses(
        ...
        create_test_reminder_status(
            reminder_id=reminder1.id, status=ReminderStatusEnum.DONE.value
        )
        create_test_reminder_status(
            reminder_id=reminder2.id, status=ReminderStatusEnum.PENDING.value
        )

...
from app.schemas.reminder_schema import ReminderResponse, ReminderCreate, ReminderUpdate
...
async def get_all_reminder_by_caregiver(
    ...
    Retrieves all reminders that were created by the currently authenticated caregiver.
    
    Args:
        caregiver_id (str): ID of the authenticated caregiver (extracted from JWT token)
...
async def get_all_reminder_by_user(
    ...
    Retrieves all reminders that are assigned to the currently authenticated user.
    
    Args:
        user_id (str): ID of the authenticated user (extracted from JWT token)
        reminder_facade (ReminderFacade): Reminder service facade
...
async def get_reminder(
    ...
    """
    try:
        reminder = reminder_facade.get_reminder(reminder_id)
...
async def create_reminder(
    ...
    Creates a new reminder for a user that the authenticated caregiver has access to.
    The caregiver must be associated with the user to create reminders for them.
    
    Args:
        request (ReminderCreate): Reminder data (title, description, scheduled_at, user_id)
        caregiver_id (str): ID of the authenticated caregiver (extracted from JWT token)
        reminder_facade (ReminderFacade): Reminder service facade
...
async def update_reminder(
    ...
    Updates a reminder's details. Only the authenticated caregiver who created
    the reminder can update it. Supports partial updates (only provided fields are updated).
    ...
    try:
        ...
        # Perform update
        updated_reminder = reminder_facade.update_reminder(reminder.id, update_data)
        
        return ReminderResponse(
            id=updated_reminder.id,
            title=updated_reminder.title,
            description=updated_reminder.description,
            scheduled_at=updated_reminder.scheduled_at,
            user_id=updated_reminder.user_id,
            caregiver_id=updated_reminder.caregiver_id,
...
async def delete_reminder(
    ...
    Permanently deletes a reminder. Only the authenticated caregiver who created
    ...
    try:
        ...
        # Delete the reminder
        success = reminder_facade.delete_reminder(reminder_id)
